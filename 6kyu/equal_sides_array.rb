...
# N is equal to the sum of the integers to the right of N. If there is no
...
def find_even_index(arr)
  ...
  # arr.each_index do |idx|
    return idx if arr[0..idx].inject(:+) == arr[idx..-1].inject(:+)
  end
  -1
end
...
def find_even_index(arr)
  arr.detect(lambda{-1}).with_index do |_,idx|
    return idx if arr[0..idx].inject(:+) == arr[idx..-1].inject(:+)
  end
end
